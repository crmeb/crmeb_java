<template>
  <div class="storeEdit">
    <el-form>
            ...
            <template #default="scope">
              <div class="upLoadPicBox">
                <div v-if="formValidate.image" class="pictrue tabPic">
                  <el-image :preview-src-list="isDisabled ? [scope.row.image] : []" preview-teleported :src="scope.row.image" />
                </div>
      ...
      >
        <el-table :data="ManyAttrValue" border class="tabNumWidth">
...
const tempRoute = ref({});
// 以下在原 methods 中被引用但未在 data 中声明（保留原逻辑）
...
function success() {
  ...
  if (formValidate.specType) {
    ...
    stockAddApi({ attrValueList: arr, id: ManyAttrValue.value[0].productId }).then((res) => {
      emit('sucess');
    });
  } else {
    OneattrValue.value.forEach((val) => {
      arr.push({ addStock: val.addStock, id: val.id, version: 0 });
    });
    stockAddApi({ attrValueList: arr, id: OneattrValue.value[0].productId }).then((res) => {
      emit('sucess');
    });
  }
}
...
function getInfo(id) {
  ...
  productDetailApi(id)
    .then(async (res) => {
      ...
      productGetRule();
      if (info.specType) {
        formValidate.attr = info.attr.map((item) => {
          return {
        ...
        let manyAttr = attrFormat(formValidate.attr);
...
function watCh(val) {
  ...
  ManyAttrValue.value.forEach((val, index) => {
...
if (formValidate.specType) {
  watch(
    () => formValidate.attr,
    (val) => {
      if (formValidate.specType) watCh(val); //重要！！！
    },
