...
export default {
  ...
  methods: {
    ...
    formatCategory(list) {
      return (Array.isArray(list) ? list : []).map((item) => {
        return {
          ...
          children: item.children && item.children.length ? this.formatCategory(item.children) : undefined,
        };
      });
    },
    //商品分类
    sliderChange(e, index) {
      ...
      this.configData.list[index].classPage.id = e;
      const category = this.findCategory(this.treeSelect, e);
      this.configData.list[index].classPage.name = category ? category.label : '';
    },
    findCategory(list = [], id) {
      for (const item of list) {
        if (item.id == id || item.value == id) return item;
        const child = this.findCategory(item.children || item.child || [], id);
