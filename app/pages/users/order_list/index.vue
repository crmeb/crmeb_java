...
import { useColor } from '@/composables/useColor.js';
	...
	function getNavH(marTopVal){
		marTop.value = marTopVal;
	}
	function onLoadFun() {
		getOrderData();
		getOrderList();
	 ...
	 */
	onLoad((options) => {
		if (options.status) orderStatus.value = options.status;
	});
	/**
	 * 获取订单统计数据
	 * 
				...
				if(res.confirm) {  
					...
					orderCancel(order_id).then(res => {
						...
						return proxy.$util.Tips({
							...
							orderData.value.unpaid_count = orderData.value.unpaid_count - 1;
							getOrderData();
						});
					}).catch(err => {
						return proxy.$util.Tips({
	 ...
	 * 去订单详情
	 */
	function goOrderDetails(order_id,status) {
		if (!order_id) return proxy.$util.Tips({
			title: '缺少订单号无法查看订单详情'
		});
		// #ifdef MP
