...
<script setup>
	...
	onMounted(() => {
		nextTick(() => {
			nodesRef(`.${props.selector}`).then((res) => {
				if(res && res[0]){
					winHeight.value = res[0].height + Math.abs(res[0].top)
				}
			});
			!isPreload() && selectorQuery()
		})
	});
	...
	function isPreload(init) {
		let preloadData = props.preloadData || []
	...
	async function selectorQuery() {
		...
		for (let item of skeletonType) {
			...
			className = `.${props.selector} >>> .${item}`;
			if (~'rect_circular_fillet'.indexOf(item)) {
				className = `.${props.selector} >>> .${props.selector}-${item}`;
			}
			// #endif
			await nodesRef(className).then((res) => {
				res.map(d => {
					d.skeletonType = item
				})
	...
	async function nodesRef(className) {
