<template>
  <div id="app">
    <router-view v-if="isRouterAlive" />
    <Setings ref="setingsRef" />
  </div>
</template>

<script setup>
// +----------------------------------------------------------------------
// | CRMEB [ CRMEB赋能开发者，助力企业发展 ]
// +----------------------------------------------------------------------
// | Copyright (c) 2016~2023 https://www.crmeb.com All rights reserved.
// +----------------------------------------------------------------------
// | Licensed CRMEB并不是自由软件，未经许可不能去掉CRMEB相关版权
// +----------------------------------------------------------------------
// | Author: CRMEB Team <admin@crmeb.com>
// +----------------------------------------------------------------------
import { ref, provide, watch, onMounted, onUnmounted, nextTick } from 'vue';
import { useRoute } from 'vue-router';
import { Local } from '@/utils/storage.js';
import Setings from '@/layout/navBars/breadcrumb/setings.vue';
import bus from '@/utils/bus';
import { useMenuStore } from '@/store/modules/menu';
import { useUserStore } from '@/store/modules/user';
import { useThemeConfigStore } from '@/store/modules/themeConfig';

const route = useRoute();
const menuStore = useMenuStore();
const userStore = useUserStore();
const themeConfigStore = useThemeConfigStore();

const isRouterAlive = ref(true);
const setingsRef = ref(null);

// provide reload 方法供子组件调用以重载当前视图
const reload = () => {
  isRouterAlive.value = false;
  nextTick(() => {
    isRouterAlive.value = true;
  });
};
provide('reload', reload);

// 监听路由 控制侧边栏显示 标记当前顶栏菜单（如需要）
watch(
  route,
  (to, from) => {
    const onRoutes = to.meta.activeMenu ? to.meta.activeMenu : to.meta.path;
    menuStore.setActivePath(onRoutes);
    if (to.name == 'crud_crud') {
      userStore.oneLvRoutes.map((e) => {
        if (e.path === to.path) {
          to.meta.title = e.title;
        }
      });
    }
    //优惠券、秒杀活动
    if (['creatProduct', 'CreatCoupon', 'CreatSeckill', 'CreatTag', 'border', 'articleCreat'].includes(to.name)) {
      let routePath = to.matched[1].path.split(':')[0];
      userStore.oneLvRoutes.map((e) => {
        if (routePath.indexOf(e.path) != -1) {
          to.meta.title = `${e.title} ${to.params.id ? 'ID:' + to.params.id : ''}`;
        }
      });
    }
    //个人中心、修改密码
    if (['MaintainUser', 'MaintainUpdate'].includes(to.name)) {
      bus.emit('oneCatName', '控制台');
    }
  },
);

// 布局配置弹窗打开
const openSetingsDrawer = () => {
  bus.on('openSetingsDrawer', () => {
    setingsRef.value && setingsRef.value.openDrawer();
  });
};

// 获取缓存中的布局配置
const getLayoutThemeConfig = () => {
  if (Local.get('JavaPlatThemeConfigPrev')) {
    themeConfigStore.setThemeConfig(Local.get('JavaPlatThemeConfigPrev'));
    document.documentElement.style.cssText = Local.get('JavaPlatThemeConfigStyle');
  } else {
    Local.set('JavaPlatThemeConfigPrev', themeConfigStore.themeConfig);
  }
};

onMounted(() => {
  openSetingsDrawer();
  getLayoutThemeConfig();
});

onUnmounted(() => {
  bus.off('openSetingsDrawer');
});
</script>
