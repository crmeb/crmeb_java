...
import { cloneConfig, fillMissingConfig } from '@/views/design/theme_editor/utils/mergeConfigDefaults';

defineOptions({ name: 'common_wrapper' });
...
const props = defineProps({
  ...
  // Optional override props for styling
  bottomBgColor: { type: String, default: '' },
  marginConfig: { type: Object, default: null },
  paddingConfig: { type: Object, default: null },
  componentBgConfig: { type: Object, default: null },
  zIndexConfig: { type: Object, default: null },
  borderConfig: { type: Object, default: null },
  shadowConfig: { type: Object, default: null },
  fillet: { type: Object, default: null },
  borderRadius: { type: [String, Number], default: '' },
  textColor: { type: Object, default: null },
});

const mobildConfigStore = useMobildConfigStore();

function getCompletedConfig(config) {
  if (!config || config.timestamp === undefined || !config.name) return false;
  const current = mobildConfigStore.defaultArray[config.timestamp];
  if (!current) return cloneConfig(config);

  const completed = fillMissingConfig(cloneConfig(current), config);
  return JSON.stringify(current) === JSON.stringify(completed) ? null : completed;
}

watch(
  () => props.config,
  (config) => {
    const completed = getCompletedConfig(config);
    if (!completed) return;
    mobildConfigStore.UPDATEARR({
      num: config.timestamp,
      val: completed,
    });
  },
...
const boxStyle = computed(() => {
  const config = props.config || {};
  const marginConfig = props.marginConfig ||
    config.marginConfig || {
      val: 0,
      valList: [{ val: 0 }, { val: 0 }, { val: 0 }, { val: 0 }],
    };
  const paddingConfig = props.paddingConfig ||
    config.paddingConfig || {
      val: 0,
      valList: [{ val: 0 }, { val: 0 }, { val: 0 }, { val: 0 }],
    };
  // bottomBgColor logic moved to wrapperStyle
  const componentBgConfig = props.componentBgConfig || config.componentBgConfig;
  const zIndexConfig = props.zIndexConfig || config.zIndexConfig;
  const borderConfig = props.borderConfig || config.borderConfig;
  const shadowConfig = props.shadowConfig || config.shadowConfig;
  const fillet = props.fillet || config.fillet;
  const borderRadius = props.borderRadius || (config.borderRadius ? config.borderRadius : '0');
  const style = {
  ...
  // Text Color
  if (props.textColor && props.textColor.color) {
    style.color = props.textColor.color[0].item;
  }
